...
- **SECURITY/CORRECTNESS BACKLOG — adversarial code review (2026-07-30). ALL 7 FIXED + DEPLOYED + VERIFIED LIVE 2026-07-31** (image `f14e7394`, prev `17fc5372`). Backend byte-compiles; #2's fix unit-tested (duplicate flagged, single copy clean, basics exempt, ban-reason dedup). Live checks: #1 — `/api/search`·`/autocomplete`·`/cards/{id}` now **401** unauthenticated (were 200), **200** with a session; #2 — re-add after setting a printing yields **1 row** (was 2); startup clean (both scheduler jobs incl. the #6 reaper registered, #4 guard passed on prod's real secret). Ranked worst-first:
...
- **DONE — mobile/responsive pass (2026-07-31, commit `be8b538`).** The whole app is now usable on phones (≤767px), not just desktop — two-row header, single-column board, full-screen sheets for the search/stats rails and all modals, tap-first editing (card panel), category reorder via ↑↓ buttons. Foundation + per-screen work in `DESIGN.md §6.4` (updated). Built with a server-side Playwright screenshot loop (Vite + Playwright containers on LavenderTown); adversarially reviewed (10 findings fixed, 2 refuted); one low a11y nit deferred (header keyboard focus order). **Ships with the next server rebuild** (same redeploy as the security fixes — not yet deployed).
