...
- **Plan approved. Build underway on branch `deckbuilder-build`.** **Phases 0–4 done + verified on the server (2026-07-18):** scaffold · card data (38k cards / 109k printings synced) · search (Scryfall proxy + adapter + compiler) · auth (username invites, JWT, admin) · decks (CRUD, legality, categories, templates, share). Backend runs on `deckbuilder:8099` (server localhost — not yet publicly proxied). **NEXT = Phase 5 (UI)**, then §6 header/import-export, then §7 deploy. Migrations at `0004`.
